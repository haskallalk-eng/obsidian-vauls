---
tags: [project, socibot, mindrails]
status: active
parent: "[[Mindrails/Overview|Mindrails]]"
created: 2026-04-18
---

# Socibot — Social Media Bot

> Ein Produkt von [[Mindrails/Overview|Mindrails]].

## Was ist es
Social Media Bot der automatisiert Content erstellt + postet.

## Wo liegt der Code
`Desktop/Social media Bot`

## Status 2026-04-18
- Video Engine v4.0 implementiert
- Test-Run ausstehend

## Offene Themen
- [ ] Test-Run durchziehen
- [ ] (weitere hier)

## Aktive Themen
- [[Socibot/Kanban|📋 Socibot Kanban Board]] — Tasks & Research

## Modul-Dokumentation (Stand 2026-04-27)

Code-belegte Bestandsaufnahme aller Module + Layer + Routes-Map + Findings:

- [[Socibot/modules/00-Overview|00 Topology]] — Stack, LOC, Sidebar-IA
- [[Socibot/modules/20-Summary|20 Master-Summary]] — alles auf einen Blick
- [[Socibot/modules/14-Findings|14 Findings]] — Code-belegte Risiken
- Layer: [[Socibot/modules/15-Layer-Frontend|Frontend]] · [[Socibot/modules/16-Layer-Backend|Backend]] · [[Socibot/modules/17-Layer-Database|Database]]
- Cross-cutting: [[Socibot/modules/18-Routes-Map|Routes-Map]] · [[Socibot/modules/19-Connections|Connections]]

## Verwandt
- [[Daily/2026-04-18]] — Test-Run als Next-Step gelistet
- [[Daily/2026-04-27]] — Sidebar-Refactor + Modul-Doku-Session
